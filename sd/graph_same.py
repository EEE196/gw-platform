...
segments = [
    ...
    range(segment_length, 2 * segment_length),
    range(2 * segment_length, len(co2_ppm))
]
...
segment_labels = ['UAV ON, Enclosed', 'UAV OFF, Enclosed', 'UAV OFF, Not Enclosed']  # Custom segment labels
...
fig.suptitle('Experiment 2: Test 1', fontsize=16)
...
for ax, variable, label in zip(axes, variables, labels):
    ...
    for i, segment in enumerate(segments):
        ...
        line = ax.plot(segment, variable[segment], color=segment_colors[i], label=segment_label)
        ...
        median_line = ax.axhline(y=segment_median, color=segment_colors[i], linestyle='--',
                                 xmin=segment.start / len(co2_ppm), xmax=segment.stop / len(co2_ppm))

        # Calculate the x-coordinate for the annotation
        x_coord = (segment.start + segment.stop) / 2
